window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SettingsSection() {
      ...
      function copyUpdateCmd() {
        try {
          if (navigator && navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText("dsh plugin update dsh-preset-prompts@latest");
          }
        } catch (err) {}
      }
      ...
      if (hasUpdate) {
        children.push(React.createElement("div", { className: "askit-update-tip", key: "uptip" },
          "终端执行 dsh plugin update dsh-preset-prompts@latest 后重启 dsh web",
    ...
    const CSS = [
      ...
      ".askit-sp-ghost { background: transparent; border: 1px solid var(--dsw-alias-border-l2); color: var(--dsw-alias-label-secondary); border-radius: 6px; padding: 2px 10px; cursor: pointer; font-size: 12px; }",
