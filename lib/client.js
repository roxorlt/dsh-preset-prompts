  ...
  factory: (require) => {
    ...
    function SettingsSection() {
      ...
      function checkUpdate() {
        fetch("https://registry.npmjs.org/dsh-preset-prompts/latest?ts=" + Date.now(), { cache: "no-store" })
          .then((r) => r.json())
          .then((info) => {
            ...
            } else if (newerThan(latest, pluginVersion || "0.1.0")) {
              showToast("发现新版本 v" + latest);
            } else {
              showToast("已是最新版本");
            }
          })
          .catch(() => showToast("检查更新失败，请稍后再试"));
      }
    ...
    const CSS = [
      ...
      ".askit-panel { z-index: 2; box-sizing: border-box; position: absolute; bottom: calc(100% + 4px); left: 0; width: min(620px, calc(100vw - 48px)); height: 340px; overflow: hidden; border: 0; background: var(--dsw-specific-menu); box-shadow: var(--dsw-shadow-lv3); border-radius: 12px; padding: 4px; color: var(--dsw-alias-label-primary); cursor: default; font-size: 13px; line-height: 20px; display: flex; font-family: var(--dsw-font-family, inherit); outline: none; }",
      ".askit-side { width: 224px; flex: none; overflow-y: auto; border-right: 0.5px solid var(--dsw-alias-border-inverted); padding: 2px; box-sizing: border-box; }",
      ...
      ".askit-title { flex: 1; font-weight: 600; font-size: 13px; line-height:20px; color: var(--dsw-alias-label-primary); }",
      ".askit-close { background: transparent; border: none; color: var(--dsw-alias-label-secondary); cursor: pointer; font: var(--dsw-font-xs-13); border-radius: 6px; padding: 2px 6px; }",
      ".askit-close:hover { color: var(--dsw-alias-label-primary); background: var(--dsw-alias-interactive-bg-hover); }",
      ".askit-error { color: var(--dsw-alias-state-error-primary); margin-bottom: 8px; font: var(--dsw-font-xxs-12); }",
      ".askit-empty { color: var(--dsw-alias-label-tertiary); font: var(--dsw-font-xxs-12); padding: 16px 8px; }",
      ".askit-fields { flex: none; }",
      ".askit-field { margin-bottom: 8px; }",
      ".askit-input { width: 100%; box-sizing: border-box; border: 0.5px solid var(--dsw-alias-border-l2); color: var(--dsw-alias-label-primary); background: transparent; border-radius: 8px; outline: none; padding: 6px 8px; font-size: 13px; line-height:20px; font-family: var(--dsw-font-family, inherit); }",
      ".askit-input:focus { border-color: var(--dsw-alias-brand-primary); }",
      ".askit-preview { flex: 1; min-height: 96px; width: 100%; box-sizing: border-box; border: 0.5px solid var(--dsw-alias-border-l2); color: var(--dsw-alias-label-secondary); background: transparent; border-radius: 8px; padding: 6px 8px; font-size: 12px; line-height: 18px; resize: none; outline: none; font-family: var(--dsw-font-family, inherit); margin-top: 4px; }",
      ".askit-actions { margin-top: 10px; display: flex; justify-content: flex-end; flex: none; }",
      ".askit-inject { background: var(--dsw-alias-button-info-fill, var(--dsw-alias-brand-primary)); color: var(--dsw-alias-label-primary-foreground); border: none; border-radius: 999px; corner-shape: round; padding: 5px 14px; cursor: pointer; font-size: 13px; line-height:20px; font-weight: 500; }",
      ...
      ".askit-sp-title { flex: 1; font-weight: 600; font-size: 14px; line-height:22px; color: var(--dsw-alias-label-primary); }",
      ".askit-sp-group { color: var(--dsw-alias-label-tertiary); font-size: 11px; line-height: 16px; padding: 12px 8px 2px; border-top: 0.5px solid var(--dsw-alias-border-l1); margin-top: 6px; }",
      ...
      ".askit-sp-ghost { background: transparent; border: 0.5px solid var(--dsw-alias-border-l2); color: var(--dsw-alias-label-secondary); border-radius: 6px; padding: 2px 10px; cursor: pointer; font: var(--dsw-font-xxs-12); white-space: nowrap; flex: none; }",
      ...
      ".askit-var-row { display: flex; align-items: center; gap: 6px; border: 0.5px solid var(--dsw-alias-border-l2); border-radius: 8px; padding: 4px 8px; margin-bottom: 6px; font: var(--dsw-font-xxs-12); }",
      ...
      ".askit-sugg { position: absolute; top: 100%; left: 0; right: 0; z-index: 5; margin-top: 2px; border: 0; background: var(--dsw-specific-menu); box-shadow: var(--dsw-shadow-lv3); border-radius: 8px; padding: 2px; max-height: 160px; overflow: auto; }",
      ".askit-sugg-item { display: block; width: 100%; box-sizing: border-box; background: transparent; border: none; border-radius: 6px; padding: 4px 8px; cursor: pointer; text-align: left; font: var(--dsw-font-xxs-12); color: var(--dsw-alias-label-primary); }",
      ".askit-sugg-item:hover { background: var(--dsw-alias-interactive-bg-hover); }",
      ".askit-body { min-height: 120px; width: 100%; box-sizing: border-box; border: 0.5px solid var(--dsw-alias-border-l2); border-radius: 8px; outline: none; padding: 8px; font-size: 13px; line-height: 24px; color: var(--dsw-alias-label-primary); background: transparent; white-space: pre-wrap; word-break: break-word; font-family: var(--dsw-font-family, inherit); }",
      ...
      ".askit-foot { display: flex; align-items: center; gap: 8px; padding: 12px 8px 4px; margin-top: 8px; border-top: 0.5px solid var(--dsw-alias-border-l1); }",
      ".askit-foot-ver { color: var(--dsw-alias-label-tertiary); font: var(--dsw-font-xxs-12); }",
      ".askit-foot-update { color: var(--dsw-alias-state-warn-primary); font: var(--dsw-font-xxs-12); }",
      ".askit-author { color: var(--dsw-alias-label-secondary); font: var(--dsw-font-xxs-12); text-decoration: none; margin-left: auto; }",
      ...
      ".askit-addcard { width: 100%; box-sizing: border-box; border: 1px dashed var(--dsw-alias-border-l2); background: transparent; color: inherit; font: inherit; cursor: pointer; border-radius: 12px; padding: 12px; display: flex; flex-direction: column; align-items: flex-start; gap: 4px; text-align: left; transition: background var(--ds-transition-duration-fast), border-color var(--ds-transition-duration-fast); margin-top: 6px; }",
      ...
      ".askit-addcard-chip { border: 0.5px solid var(--dsw-alias-border-l2); background: var(--dsw-alias-bg-layer-2); width: 28px; height: 28px; color: var(--dsw-alias-label-tertiary); border-radius: 8px; flex: none; display: inline-flex; justify-content: center; align-items: center; font-size: 16px; line-height: 1; }",
      ...
      "@media (prefers-reduced-motion: reduce) { [class^=\"askit-\"], [class*=\" askit-\"] { transition: none; animation: none; } }",
    ].join("\n");
